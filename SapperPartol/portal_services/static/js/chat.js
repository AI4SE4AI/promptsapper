...
function generateDialog(who, msg){
    ...
    <div class="dialog-portrait">
        <img src="/static/images/role/${name}.png" class="dialog-portrait-img" alt="">
        <p class="dialog-portrait-name">${name}</p>
    </div>
    <div class="dialog-msg-wrapper">
        <p class="dialog-msg">${msg}</p>
    </div>
</div>`; // 文字转语音，这部分放到 <p class="dialog-msg">里：<button class="dialog-msg-btn btn btn-msg" onclick="text2audio('${voice_msg}')">
...
function show_loading() {
    ...
    convWrapper.append(`<div class="conversation-dialog dialog-sapper" data-role="sapper">
        <div class="dialog-portrait">
            <img src="/static/images/role/${role}.png" class="dialog-portrait-img" alt="">
            <p class="dialog-portrait-name">${role}</p>
        </div>
...
function ajax_chat(msg){
            ...
            type: 'post',
                ...
                if (res["tokens"] <= 0) {
                    ...
                    show_loading(); // 显示等待状态
                    var source = new EventSource("/chat/" + role + "?query=" + msg + "&history_id=" + current_history_id + "&new_old=" + new_old);
      ...
      if (recognition) {
        ...
        recognition.onstart = function() {
          ...
          // 找到id为speak_i的元素，往里面添加文本'···'
            document.getElementById('speak_i').innerHTML = document.getElementById('speak_i').innerHTML + '···';
        };
